...
menu() { 
local olpwd="$PWD" size=($(stty size)); for i in $(seq $size); do echo; done; 
echo -ne '\e[s\e[H\e[J'; 
local green='\e[92m' cyan='\e[36m' re='\e[0m'; 
local dim='\e[2m' bold='\e[1m' c2='\e[36m--\e[0m';
local command=bash; [ "$2" ]&& command="$2"; hello=hello; [ $3 ]&& hello=$3; 
...
if [[ $1 ]]; then cd $1; OPTIONS_VALUES=($(ls -p|grep -v /)); else 
#OPTIONS_VALUES=($(ls -p $PWD/$1)); fi; 
OPTIONS_VALUES=($@); 
fi; 
...
local retval=$1; IFS=';' read -r -a options <<< "$2"; if [[ -z $3 ]]; 
